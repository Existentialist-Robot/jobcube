...
- `tools/` — [`lint_skills.py`](tools/lint_skills.py), [`security_guards.py`](tools/security_guards.py) (fails if anything personal is tracked), [`check_conflict_markers.py`](tools/check_conflict_markers.py) (a committed merge marker passes every other gate), and [`check_upstream_updates.py`](tools/check_upstream_updates.py) (previews which personalized files an upstream change would touch). All four run in CI.
